...
# ============================================================
# Per-chat dashboard message tracking (memory only, no GitHub)
# ============================================================

DASHBOARD_MSG_IDS = {}

async def send_dashboard(chat, forum, snapshot):
    text = build_dashboard_from_snapshot(snapshot)

    old_msg_id = DASHBOARD_MSG_IDS.get(chat)
    if old_msg_id:
        try:
            await bot.delete_message(chat_id=int(chat), message_id=int(old_msg_id))
        except Exception:
            pass

    msg = await send_text(chat, text, forum=forum)
    DASHBOARD_MSG_IDS[chat] = msg.message_id
    return msg
...
async def handle(update: Update):
    ...
    if low in ["/status", "📊 status"]:
        ...
        await send_dashboard(chat, forum, snapshot)
        return
