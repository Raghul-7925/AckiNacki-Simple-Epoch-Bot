...
async def handle(update: Update):
    ...
    if low in ["/analysis", "📈 analysis"]:
        ...
        for h in hist[-30:]:
            ...
            # Show exact reset block for known epochs (e.g., Epoch 204 at 53,448,000)
            reset_block = h.get("reset_block")
            reset_timestamp = h.get("reset_timestamp")
            
            if kind == "auto_reset":
                out += f"📅 Epoch {epoch_no} | Auto Reset\n"
                out += f"• Start Block: {h['epoch_start_block']:,}\n"
                out += f"• Start Time: {h['date']} | {h['epoch_start_ist']} | UTC: {h['epoch_start_utc']}\n"
                out += f"• Reset Block: {reset_block:,}\n"
                
                # Show timestamp-based reset time if available
                if reset_timestamp:
                    reset_dt = datetime.fromtimestamp(reset_timestamp, IST)
                    out += f"• Reset Time: {reset_dt.strftime('%d %b %Y')} | {reset_dt.strftime('%I:%M %p')} | UTC: {reset_dt.astimezone(UTC).strftime('%H:%M')}\n"
                else:
                    out += f"• Reset Time: {h['date']} | {h['reset_ist']} | UTC: {h['reset_utc']}\n"
                
                out += f"• Epoch Duration: {h.get('epoch_duration', epoch_duration_text(block_sec))}\n\n"
            else:
                out += f"📅 Epoch {epoch_no} | Manual Set\n"
                out += f"• Start Block: {h['epoch_start_block']:,}\n"
                out += f"• Start Time: {h['date']} | {h['epoch_start_ist']} | UTC: {h['epoch_start_utc']}\n"
                out += f"• Reset Block: {reset_block:,}\n"
                out += f"• Reset Time: {h['date']} | {h['reset_ist']} | UTC: {h['reset_utc']}\n"
