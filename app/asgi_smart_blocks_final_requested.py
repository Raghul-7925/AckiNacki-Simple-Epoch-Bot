...
async def get_live_block_snapshot():
    """
    Smart block fetching with timestamps (from Karim's logic).
    Uses edges structure and calculates block rate from gen_utime.
    """
    ...
    # Smart block rate calculation using timestamps (Karim's method)
...
def update_epoch_anchor(state, epoch_no, start_block, block_timestamp):
    """
    Track epoch anchors with exact timestamps (from Karim's logic).
    Stores when each epoch actually started based on block gen_utime.
    """
...
def sync_epoch_state(state, current_block, current_timestamp=None, block_sec=None):
    ...
    while True:
        ...
        # Calculate actual epoch duration using timestamps if available
        epoch_key = str(epoch_no)
        prev_anchor = state.get("epoch_anchors", {}).get(epoch_key)

        if prev_anchor and current_timestamp:
            # Use actual timestamp-based duration
            actual_duration_sec = current_timestamp - prev_anchor["start_timestamp"]
            epoch_duration = format_duration(actual_duration_sec)
        else:
        ...
        add_history(state, {
            ...
            "start_block": start,
            "date": epoch_start_dt.strftime("%d %b %Y"),
            "start_date": epoch_start_dt.strftime("%d %b %Y"),
            "start_time": epoch_start_dt.strftime("%I:%M %p"),
            "start_utc": epoch_start_dt.astimezone(UTC).strftime("%H:%M"),
            "reset_block": reset,
            "reset_date": epoch_reset_dt.strftime("%d %b %Y"),
            "reset_time": epoch_reset_dt.strftime("%I:%M %p"),
            "reset_utc": epoch_reset_dt.astimezone(UTC).strftime("%H:%M"),
            "epoch_duration": epoch_duration,
            "reset_timestamp": current_timestamp if current_timestamp else int(epoch_reset_dt.timestamp()),
        })

        state["epoch_start_ts"] = int(epoch_reset_dt.timestamp())
        state["start_block"] = reset

        # Update anchor for next epoch
        next_epoch_no = get_epoch_no_from_start(reset)
        if current_timestamp:
            update_epoch_anchor(state, next_epoch_no, reset, current_timestamp)
...
async def handle(update: Update):
    ...
    if low in ["/analysis", "📈 analysis"]:
        ...
        for h in hist[-30:]:
            ...
            start_block = h.get("start_block", h.get("epoch_start_block", 0))
            start_date = h.get("start_date", h.get("date", ""))
            start_time = h.get("start_time", h.get("epoch_start_ist", ""))
            start_utc = h.get("start_utc", h.get("epoch_start_utc", ""))

            reset_block = h.get("reset_block", 0)
            reset_date = h.get("reset_date", h.get("date", ""))
            reset_time = h.get("reset_time", h.get("reset_ist", ""))
            reset_utc = h.get("reset_utc", h.get("epoch_start_utc", ""))

            out += f"📅 Epoch {epoch_no} | {'Auto Reset' if kind == 'auto_reset' else 'Manual Set'}\n"
            out += f"• Start Block: {start_block:,}\n"
            out += f"• Start Time: {start_date} | {start_time} | UTC: {start_utc}\n"
            out += f"• Reset Block: {reset_block:,}\n"
            out += f"• Reset Time: {reset_date} | {reset_time} | UTC: {reset_utc}\n"
            out += f"• Epoch Duration: {h.get('epoch_duration', epoch_duration_text(block_sec))}\n\n"
