...
async def update_dashboard(chat, forum, snapshot, store, sha):
    """
    Try to EDIT the two existing messages stored in GitHub.
    If either edit fails (message was deleted), fall back to send_fresh_dashboard.
    Only saves to GitHub when it actually had to create new messages.
    """
    pins = get_chat_pins(store, chat)
    ...
    if pin_msg_id:
        try:
            ...
            print(f"edit pin failed ({pin_msg_id}): {e}")

    # Try editing dashboard message
    if dashboard_msg_id:
        try:
            await bot.edit_message_text(
                chat_id=int(chat),
            ...
            print(f"edit dashboard failed ({dashboard_msg_id}): {e}")

    # If either message is gone, recreate both
    if not pin_ok or not dash_ok:
